...
def generate_learning_dataset():
    ...
    genre_transform = file.replace({'genre': replace_dict})
    undersample_list = []
    ...
    shuffled.tail(
        dataset_size-train_index).to_csv(LEARNING_DATASET_TEST_PATH, index=False)
    shuffled.head(int(train_index * 0.4)
                  ).to_csv(LEARNING_SMALL_DATASET_TRAIN_PATH, index=False)
